...
def open_camera():
    """
    Detect and open a camera.
    Priority: external cameras first (indices 1-5), then laptop built-in (0).
    Each candidate is verified by reading a test frame to confirm it actually works.
    """
    print("[Vision] Scanning for cameras...")

    # --- Phase 1: Try external cameras (index 1-5) ---
    for i in range(1, 6):
        print(f"[Vision]   Trying camera index {i} (external)...")
        ...
        if cap.isOpened():
            ...
            if ret and test_frame is not None:
                print(f"[Vision] [OK] External camera FOUND at index {i}")
                return cap
            else:
                print(f"[Vision]   Index {i} opened but no frame -- skipping.")
                cap.release()
        else:
            cap.release()

    # --- Phase 2: Fallback to laptop built-in webcam (index 0) ---
    ...
    if cap.isOpened():
        ...
        if ret and test_frame is not None:
            ...
            print("[Vision]   Laptop webcam opened but no frame -- releasing.")
            cap.release()
    else:
        cap.release()
